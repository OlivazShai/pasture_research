...
## Municipality intersections

### Municipalities with biomes

I intersect municipalities and biomes, defining dummies whether each biome covers some area in the respective municipality.

```{r}
muni_with_biomes <- bind_cols(
  municipalities,
...
This is not necessary for the project, but is kept in case it comes handy.

### Biome areas in municipalities

First, I calculate the area of each municipality.
...
And with the intersections, I make a table with the biome areas in the municipalities, so each biome is in a column.

```{r}
municipality_biome_area <- intersections |> 
  as_tibble() |> 
  select(
    code_muni,
    name_muni,
...
Now, I find the predominant biome in each municipality, defined as the biome with the largest share of the municipality's area.

```{r}
largest_biome_intersection <- intersections |>
  group_by(
    code_muni
  ) |>
  ...
  mutate(
    ...
    percent_covered = 100 * (area_intersection / area_muni)
  ) |>
  select(
    code_muni,
    largest_biome = name_biome,
    area_intersection,
    percent_covered
  )
```
...
```{r}
largest_biome_intersection |> 
  group_by(largest_biome) |> 
  summarise(
    biomes = n()
  )
```
