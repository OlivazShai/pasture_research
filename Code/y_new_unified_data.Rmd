...
m1 <- feols(
  data = joined_data,
  cattle_heads_big/area_amc ~ sw(1, si_mean + ma + I(pasture/area_amc*100)) | csw(1,main_biome,code_state) | local_price + local_price_2 ~ ssiv + ssiv_2,
...
```{r}
ext_data <- transition_by_amc |> 
  select(
    code_amc,
    year = year_from,
    forest_agriculture, non_forest_agriculture, forest_pasture, non_forest_pasture
  ) |> 
  rowwise(
    
  ) |> 
  mutate(
    rho_1 = sum(forest_pasture + non_forest_pasture, na.rm = T) 
  ) |> 
  ungroup(
