...
m1 <- feols(
  data = joined_data,
  cattle_heads_big/area_amc ~ sw(ma_norm +high_share, ma_norm + low_share, ma_norm + low_share+mid_share, ma_norm+ I(low_share+mid_share)) | csw(1,main_biome,code_state) | local_price + local_price_2 ~ ssiv + ssiv_2,
...
m2 <- feols(
  data = joined_data,
  cattle_heads_big ~ sw(1, si_mean + ma) | csw(1,main_biome,code_state) | local_price + local_price_2 ~ ssiv + ssiv_2,
   panel.id = c("code_amc","year"),
  cluster = "code_amc"
  )
```
...
ext_data <- transition_agg |> 
  ...
  mutate(
    x3 = 0.45*(cattle_heads_big/total)**2
  )
```
...
```{r}
feols(
  data = ext_data, # |> filter(code_biome %in% c(1,3)),
  y ~  1 + ma_norm 
  + sw(high_share, low_share, low_share+mid_share, I(low_share+mid_share) ) | main_biome | x2 ~ ssiv + ssiv_2,
  cluster = "code_amc"
  ) |> 
  etable()
```
...
```{r}
feols(
  data = ext_ppm, # filter(code_biome %in% c(1,3)),
  y ~ x2 + ma_norm | main_biome,
  cluster = "code_amc"
  ) |> 
  etable()
```
