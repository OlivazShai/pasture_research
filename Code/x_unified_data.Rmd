...
```{r}
s1 <- plm(
  local_price ~ lag(chicken_price,1:2):iceberg_cost,
  data = y,
  model = "pool"
  )
...
```{r}
model_3 <- plm(
  log(cattle_heads) ~ local_price + lag(local_price) + pasture_cover | . - local_price + lag(chicken_price,1:2):iceberg_cost,
  data = y,
  model = "within",
  effect = "individual"
  )
...
```{r}
model_4 <- plm(
  log(cattle_heads) ~ local_price + lag(local_price) + pasture_cover + as.numeric(year) | . - local_price + lag(chicken_price,1:2):iceberg_cost,
  data = y,
  model = "within",
  effect = "individual"
  )
...
```{r}
model_5 <- plm(
  cattle_heads ~ local_price + lag(local_price) + pasture_cover + as.numeric(year) | . - local_price + lag(chicken_price):iceberg_cost,
  data = y,
  model = "within",
  effect = "individual"
  )
