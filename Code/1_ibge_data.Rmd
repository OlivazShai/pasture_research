...
### Biome area in each municipality

I'll also calculate the area each biome occupies in each municipality. This is ongoing! I have to deal with units, doing that soon...

```{r}
x <- sample_n(municipalities, 100)


# Perform intersection and calculate area
intersections <- st_intersection(x, biomes) %>% 
...
# Create a table with the area for each muni and biome
municipality_biome_area <- intersections %>%
  ...
  pivot_wider(
    ...
    values_fill = NA  # Fill with 0 if there's no intersection
  )
```
