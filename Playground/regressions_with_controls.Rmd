...
recovery_results <- x |> 
  mutate(
    # land area SAVED
    def_disincentive = hat - potential,
    def_incentive = potential - effective,
    def_net = hat - effective,
    
    def_disincentive_pc = def_disincentive/hat,
    def_incentive_pc = def_incentive/hat,
    def_net_pc = def_net/hat,
...
```{r}
ggplot(recovery_results) +
  geom_line(aes(y = i, x = effective_co2)) +
  labs(y = "pasture quality", x = "emissions (Gt Co2)") +
  theme_minimal()
```
...
ext_cf |> 
  mutate(
    y_biomass = tax_counterfactual(int_mod, ext_mod, ext_cf, tax = 50),
    y_hat = predict(ext_mod, newdata = ext_cf)
  ) |> 
  select(
