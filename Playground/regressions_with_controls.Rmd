...
I construct a database for the extensive margin, where I also get results from 2007 and 2018, using PPM data to expand the series one year ahead. 
...
recovery_counterfactual <- function(
  ...
  # Append to data
  int_cf_data_recovery_clean <- int_data |>
    select(
      code_amc,
      year
    ) |> 
  ...
  # Predicted
  ext_cf_data_predicted <- ext_data |> 
    inner_join(
      int_cf_data_recovery_clean,
      by = c("code_amc", "year")
    ) |> 
    # remove impossible probability for rho_3
    filter(
      rho_3 != 0
    ) |> 
  ...
  ext_cf_data_recovery_potential_full <- ext_data_complete |> 
    # increase high_share
    mutate(
      high_share = if_else(
          # in census years and the following years
        year %in% c(2006:2007, 2017:2018) 
  ...
  ext_cf_data_recovery_potential_2017 <- ext_cf_data_recovery_potential_full |> 
    # filter to census years
    filter(
      year == 2006 | year == 2017
    ) |> 
    ...
    # join with intensive margin counterfactual data
    inner_join(
      int_cf_data_recovery_clean,
      by = c("code_amc", "year")
    ) 
  ...
  ext_cf_data_recovery_effective <- ext_cf_data_recovery_potential |> 
    mutate(
      x2 = 0.45*(h_recovery)**2
    )
  
  # Append to data
  ext_cf_data_recovery_clean <- ext_cf_data_predicted |>
    select(
      code_amc, year, rho_1, rho_2, natural
    ) |> 
    mutate(
      # predicted
      y_hat = predict(ext_mod, newdata = ext_cf_data_predicted),
      # pasture recovery
      y_potential = predict(ext_mod, newdata = ext_cf_data_recovery_potential),
      # maximum potential
      y_effective = predict(ext_mod, newdata = ext_cf_data_recovery_effective)
  ...
  ext_cf <- ext_cf_data_recovery_clean |> 
    mutate(
      rho_hat = logit(
      ...
      rho_effective = logit(
        y_effective + 0.9*log(rho_2) - 0.9*gamma
        ),
    ...
    summarise(
      ...
      effective = sum(rho_effective*natural),
