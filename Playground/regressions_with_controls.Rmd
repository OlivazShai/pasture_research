...
## Real and predicted baselines

```{r}
b <- recovery_counterfactual(
    int_mod, 
    ext_mod, 
    int_data,
    ext_cf, 
  ...
  mutate(
    hat_error = hat/real -1,
    co2_error = hat_co2/real_co2 -1
  )

b_conversion <- b |> 
  select(hat, real, hat_error) |> 
  rename(
    predicted = hat,
    observed = real,
    error = hat_error
...
b_co2 <- b |> 
  select(hat_co2, real_co2, co2_error)|> 
  rename(
    predicted = hat_co2,
    observed = real_co2,
    error = co2_error
...
baseline_results |> 
  mutate(
    variable = c("Land conversion ($10^4 km^2$)", "Carbon emissions (Gt $CO_2$)")
  ) |>
  kbl(
    col.names = c(
      "Variable",
...
policy_summary |> 
  ...
  pack_rows("A: Pasture Recovery", 1, 4) |> 
  pack_rows("B: Carbon Tax", 5, 11) |>
  footnote(
