...
int_cf <- int_data |> 
  mutate(
    h_hat = fitted(int_mod),
    h_hat1 = predict(int_mod),
    h_hat2 = predict(int_mod, newdata = int_data),
    h_recovery = predict(int_mod, newdata = int_cf_data_recovery),
    high_share_old = high_share,
    high_share_cf = if_else(
      high_share > min_high_share, 
      high_share, 
      min_high_share),
...
a <- data.frame(int_data == int_cf_data_recovery)
