...
```{r}
tax_counterfactual(int_mod,ext_mod,int_data,ext_cf,
                               tax = 66)
```
...
recovery_results |> 
  ...
  theme(
    ...
    axis.title.x = element_text(
      size = 10,
      margin = margin(t = 10),
      family = "LM Sans 10"  # Change to your preferred sans-serif font
      ),
    axis.title.y = element_text(
      size = 10,
      family = "LM Sans 10"  # Ensures y-axis titles are sans-serif
      ),
    axis.text = element_text(
      family = "LM Sans 10"  # Change tick labels to sans-serif
      ),
    legend.text = element_text(
      family = "LM Sans 10",  # Change legend text to sans-serif
      size = 9
      ),
    legend.position = "bottom",
    legend.title = element_blank(),  # Remove legend title
    strip.placement = "outside",      # Ensure facet titles remain outside panels
    strip.text = element_text(
      family = "LM Sans 10",  # Panel titles in "LM Roman 10"
      size = 11.5,
      face = "plain"
      ),
...
```{r}
tax_results_smooth <- approx(x = tax_results$t, y = tax_results$tax_co2, n = 1000) |>  as_tibble()
...
combined <- bind_rows(
  tax_results |> 
    ...
    transmute(y = sav_tax_pc, x = t, facet = "tax"),
  recovery_results |> transmute(y = sav_net_pc, x = i, facet = "recovery")
  )
...
combined |> 
  ...
  labs(
    x = NULL,  # Remove global x-axis title
    y = "Y-Axis Label"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(size = 11, face = "plain", family = "LM Sans 10"),
    panel.grid.minor = element_blank(),
