...
design_matrix = model.matrix(int_mod) 

x = as.vector(design_matrix %*% coefs)

fixed_effects = predict(int_mod, fixef = T)$main_biome |> unname()

y_recovery = x + fixed_effects
```

```{r}
a <- model.matrix(int_mod) |> head(1) * coefs

predict(int_mod, newdata = int_data)
```
